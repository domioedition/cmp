<template>
  <div class="project" v-if="project">
      ...
      <hr>
      <h3>Current tasks</h3>
      <button type="button" class="btn btn-primary" v-on:click.prevent="showAddForm = !showAddForm">New task</button>

      <div v-if="showAddForm">
        <input type="text" placeholder="Title" v-model="title"><br>
        <textarea placeholder="Description" v-model="descr" rows="6" cols="60"></textarea><br>
...
<script>
  ...
  export default{
    ...
    methods:{
      createTask(){
        ...
        console.log(this.$route.params.pid)
          this.axios.post('http://172.104.252.11/public/task/1/', {
            pid: 1,
            user_id_creator: 1,
            user_id_assignee: 1,
    ...
    created(){
      ...
      axios.get('http://172.104.252.11/public/task/'+projectId+'/').then((response) => {
